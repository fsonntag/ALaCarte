...
def wet_documents(pathsfile, reader, verbose=False, comm=None):
    ...
    client = boto3.client('s3', config=botocore.client.Config(signature_version=botocore.UNSIGNED, read_timeout=TIMEOUT,
    ...
    for i, path in enumerate(paths):
        ...
        if i > position and not i % 1000:
            ...
            if verbose and not rank:
                write('\rProcessed ' + str(i) + '/' + str(
                    len(paths)) + ' Paths; Target Coverage: ' + reader.target_coverage(), comm)
...
def main(args, comm=None):
    ...
    interval = [int(args.interval[0]), int(args.interval[1])] if args.interval[1].isdigit() else [int(args.interval[0]),
                                                                                                  float(
                                                                                                      args.interval[1])]
    ...
    if args.corpus:
        ...
        for corpus in args.corpus:
            ...
            context_vectors, target_counts = corpus_documents(corpus, alc, verbose=args.verbose, comm=comm,
                                                              english=args.english, lower=args.lower)
            if args.restart:
                alc.checkpoint(0)
    else:
        write('Source Corpus: WET Files in ' + args.paths + '\n', comm)
        context_vectors, target_counts = wet_documents(args.paths, alc, verbose=args.verbose, comm=comm,
                                                       english=args.english, lower=args.lower)
        if args.restart:
            alc.checkpoint(-1)
    ...
    if M is None:
        ...
        write('Finished Learning Transform; Average Cosine Similarity: ' + str(
